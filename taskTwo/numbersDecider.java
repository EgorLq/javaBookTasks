...
public class numbersDecider {
    static int findNumbersDecider(Integer[] sourceNumbers) {
        ...
        for (Map.Entry<Integer, Integer> record : countNumber.entrySet()) {
            toSort.add(record);                                                    // запись в массиве отсортированных чисел
        }
        toSort.sort(Map.Entry.<Integer, Integer>comparingByValue().reversed()); // сортировка в обратном порядке
        for (Map.Entry<Integer, Integer> record : toSort) {
            ...
            for (int i = 0; i < value; i++) {           // цикл на подсчет кол во
                number.add(key);                                               // добавление кол -во раз встреч того или иного числа
            }

        }
        System.out.println("Числа в порядке убывания частоты встречаемости чисел: " +  number ) ; //  вывод числа
        return 0;
    }
}
