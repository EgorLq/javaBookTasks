...
public class elementsHalfScrip {
    void findElementsHalfScrip (Integer[] sourceNumbers) {
        ...
        for (int i = 1; i < numbers.length - 1; i++) { //  цикл для подсчета
            ...
            if (numbers[i] == (numbers[i - 1] + numbers[i + 1]) / 2) { //  ставим условие что если полусумма равна нашему числу то
                halfScripNumber.add(numbers[i]); //  добавляем число
            }
        }
        System.out.println("Элементы, которые равны полусумме соседних  элементов " + halfScripNumber); //  выводим число
    }
}
